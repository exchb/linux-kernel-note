...
inline void update_rq_clock(struct rq *rq)
{
	...
	// rq->clock_task应该代表的是当前任务最后执行的时间戳
	// 此后执行irq(hard && soft)
	// 当前时间按是rq->clock,减去irq time就是任务最后执行时间
	// just guess^^^^^^^^^^^^^^^^^^
...
static void resched_task(struct task_struct *p)
{
	...
	// test if flag TIF_NEED_RESCHED has  been set 
...
/*
 * Shift right and round: 模2^y然后四舍五入(二进制的四舍五入)
 */
		...
		struct load_weight *lw)
{
	...
	// 处理溢出?
	if (!lw->inv_weight) {
		if (BITS_PER_LONG > 32 && unlikely(lw->weight >= WMULT_CONST)) // max than 2^32
			lw->inv_weight = 1;
		else
...
static void activate_task(struct rq *rq, struct task_struct *p, int wakeup)
{
	if (task_contributes_to_load(p))	// 对新进程而言，非真
	...
	if (p->sched_class == rq->curr->sched_class) {
		...
		for_each_class(class) {
			...
			if (class == p->sched_class) {
				resched_task(rq->curr);	// set flag : TIF_NEED_RESCHED && broadcast to all cpu
...
void wake_up_new_task(struct task_struct *p, unsigned long clone_flags)
{
	...
	check_preempt_curr(rq, p, WF_FORK);	// 判断当前进程是否被新进程抢占
#ifdef CONFIG_SMP
	if (p->sched_class->task_woken)
		p->sched_class->task_woken(rq, p);
#endif
