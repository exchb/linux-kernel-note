...
static void update_min_vruntime(struct cfs_rq *cfs_rq)
{
	...
	// 选出rb_tree里面具有最小vruntime的进程
	// 注意: cfs_rq->curr当前是不在rb_tree里面的
	if (cfs_rq->rb_leftmost) {
		struct sched_entity *se = rb_entry(cfs_rq->rb_leftmost,
						   struct sched_entity,
						   run_node);

		if (!cfs_rq->curr)	// vruntime = cfs_rq->min_vruntime;
			vruntime = se->vruntime;
		else	// vruntime = cfs_rq->curr->vruntime;
				// 表rb_tree中最左实体和cfs_rq->curr实体的vruntime
			vruntime = min_vruntime(vruntime, se->vruntime);
	}

	// 和队列原有的min_vruntime比较，取其较大值
	// 原有的最小值已经失效了
...
static u64 sched_slice(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	...
	for_each_sched_entity(se) {
		...
		cfs_rq = cfs_rq_of(se);	// 所属的cfs队列
		load = &cfs_rq->load;

		// 将 !se->on_rq的所有se的load累加到cfs rq的load
...
enqueue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	if (se != cfs_rq->curr)
		__enqueue_entity(cfs_rq, se);	 // insert into rb-tree
}
...
static void check_preempt_wakeup(struct rq *rq, struct task_struct *p, int wake_flags)
{
	...
	// 不属于同一个调度类，当然也不属于同一个cfs_rq，不予考虑
